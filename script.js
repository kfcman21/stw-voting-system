...
function empathyVote(category, optionNumber) {
    ...
    if (!empathy[category][empathyKey]) {
        empathy[category][empathyKey] = 0;
    }
...
// 투표 요약 업데이트
function updateVoteSummaries() {
    updateCategoryVoteSummaries('see');
    updateCategoryVoteSummaries('think');
    updateCategoryVoteSummaries('wonder');
}
...
// 각 카테고리별 투표 요약 업데이트
function updateCategoryVoteSummaries(category) {
    const categoryVotes = votes[category] || {};
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        const rank2 = categoryVotes[`${i}_2`] || 0;
        const rank3 = categoryVotes[`${i}_3`] || 0;
        const totalVotes = rank1 + rank2 + rank3;
        
        const summaryElement = document.getElementById(`vote-summary-${category}-${i}`);
        if (summaryElement) {
            const totalVotesSpan = summaryElement.querySelector('.total-votes');
            const rankBreakdownSpan = summaryElement.querySelector('.rank-breakdown');
            
            if (totalVotesSpan) {
                totalVotesSpan.textContent = `총 투표: ${totalVotes}표`;
            }
            
            if (rankBreakdownSpan) {
                rankBreakdownSpan.textContent = `1순위: ${rank1}, 2순위: ${rank2}, 3순위: ${rank3}`;
            }
        }
    }
}
...
function resetVotes() {
    ...
    if (confirm('모든 투표, 하트, 공감 데이터를 초기화하시겠습니까?')) {
        Promise.all([
